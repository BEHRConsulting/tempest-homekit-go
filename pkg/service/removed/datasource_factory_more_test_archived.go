...
// Archived: datasource_factory_more_test.go
// The original test file was removed from the package to avoid duplicate test names
// during `go test ./...`. The content is preserved here for reference.
...
func (f *fakeUDPListener) GetStats() (int64, time.Time, string, string)   { return 0, time.Time{}, "", "" }
...
func (f *fakeUDPListener) ObservationChannel() <-chan weather.Observation { ch := make(chan weather.Observation); close(ch); return ch }
